...
%input: RAW BEND mat file
%output: Processed signal: bend_carrot_subxx (contain 10 cell for 10 bites),
%bend_apple_subxx (contain 10 cell for 10 bites), bend_banana_subxx (contain 10 cell for 10 bites)

clc;clear all;close all;
b1=cell2mat(struct2cell(load ('BEND_SUB01.mat'))); %change filename
ts = (0:numel(b1)-1)/128;
figure;
plot(ts,b1);
title('BEND Response');
xlabel('Time (Sec)')
ylabel('BEND response');
...
bend=filtfilt(filt,b1);
bend= rmoutliers(bend);
[XNorm, mu, stddev] = featureNormalize(bend); %normalize
%%
...
save('bend_apple_sub01.mat','A') %change filename
