...
FileName = '5B30198D.BIN';  % Change the filename accordingly
...
%Conversions
PressureData = ((AIM_Data(:,1)-1638)*(2.13e-5))-0.14; %psi
%PressureData = AIM_Data(:,1); %Pressure Sensor original data
ts = (0:numel(PressureData)-1)/fs;
subplot(2,1,1);
plot(ts,PressureData); 
axis tight;
title('AIM Pressure Sensor Responses standalone');
xlabel('Time(sec)');
ylabel('psi');
...
%%
%% save bend data
save('PRESSURE_SUB11.mat','PressureData');
